...
void load_map(GameState* state, const char* filename) {
    ...
    // Eliminé la declaración de "tile sala" que no estabas usando para mantenerlo limpio

    if (!file) {
        printf("¡Error! No se pudo abrir el archivo %s. Creando mapa vacío.\n", filename);
        // Si no encuentra el archivo, llena los bordes manualmente
        for (int r = 0; r < MAP_ROWS; r++) {
            for (int c = 0; c < MAP_COL; c++) {
                if (r == 0 || r == MAP_ROWS - 1 || c == 0 || c == MAP_COL - 1) {
                    state->map.grid[r][c] = 1;
                } else {
                    state->map.grid[r][c] = 0;
                }
            }
        }
    } else {
        // El ELSE envuelve la lectura del archivo y su cierre
        for (int r = 0; r < MAP_ROWS; r++) {
            for (int c = 0; c < MAP_COL; c++) {
...
void manejo_input(GameState* state, ALLEGRO_EVENT* event) {
    ...
    void update_game(GameState* state) {
     ...
     // movimiento del jugador usando las teclas de la estructura
        ...
        if(state->keys.up)    
          { shoot_dy = -8; intentando_disparar = true; }
